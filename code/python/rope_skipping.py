...
from sklearn.metrics import SCORERS, confusion_matrix, plot_confusion_matrix, roc_auc_score, roc_curve, plot_roc_curve, auc
...
def label_target_activity(stamp, container, window_length, right_class: bool):
    start_stamp, end_stamp = stamp[0], stamp[1]
    tag, sumforce = {}, np.asarray([sumvector(a[1], a[2], a[3]) for a in container[:500]])
    if not right_class:
...
if __name__ == '__main__':
    ...
    # rope_skipping_start_stamp, rope_skipping_end_stamp = get_timestamp_step(container_G1, rope_skipping_start_time, rope_skipping_end_time)
    ...
    #     target = True if activity == 'rope_skipping' else False
    #     tag = label_target_activity(stamp['{}'.format(activity)],
    #                                 container_A1, window_length=window_length, right_class=target)
    #     tags.update(tag)
    #
    # # feature dict
    # ft = {}
    ...
    # pd.DataFrame(ft).to_pickle(r'.\single_class\test.pickle')
    # pd.DataFrame(tags, index=[0]).to_pickle(r'.\single_class\test.pickle')
    # print('preprocess over')

    ft = pd.read_pickle(r'.\single_class\features1_w5.pickle')
    tags = pd.read_pickle(r'.\single_class\tags1_w5.pickle')
    # ft1 = pd.read_pickle(r'.\single_class\features1.pickle')
    # tags1 = pd.read_pickle(r'.\single_class\tags1.pickle')
    # ft2 = pd.read_pickle(r'.\single_class\features_sk.pickle')
    # tags2 = pd.read_pickle(r'.\single_class\tags_sk.pickle')
    ...
    df_ft = shuffle(pd.DataFrame(ft).T, random_state=1).T
    df_tags = shuffle(pd.DataFrame(tags, index=[0]).T, random_state=1)
    ...
    fpr, tpr, threshold = roc_curve(Y_test, Y_prb[:,1])
    roc_auc = auc(fpr, tpr)

    # plot the roc curve for the model
    plt.plot(fpr, tpr, linestyle='--', label='auc = {}'.format(roc_auc))
    # axis labels
