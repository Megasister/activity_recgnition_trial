...
def feature_norm(df_ft, reg=None, flatten=False):
    '''
    ...
    if flatten == False:
        ...
        for i in range(len(feature_sample)):
            each_feature = []
            for sample_no in range(len(df_ft)):
                each_feature.append(df_ft.iloc[sample_no][0][i])
            total.append(each_feature)
    ...
    '''
    new_bundle = []
    for each_feature in total:
        if reg == None:
            ft_vec = each_feature
        if reg == 'norm':
            ft_vec = normalization(each_feature)
        elif reg == 'stdr':
            ft_vec = standardization(each_feature)
        new_bundle.append(ft_vec)
    return np.array(new_bundle).T.tolist()


def get_train_test_data(ftlist, taglist, i, para, test_size, random_state, shuffle_state, flatten=False):
    ...
    X = feature_norm(df_ft, para['feature_norm'], flatten)
...
def process_data():
    ...
    # [1.   1.   1.   0.92 0.95 0.97 0.85 0.95 0.87 0.92] size=5, child 1,2,5
    # [0.93 0.97 1.   0.92 0.97 0.97 0.87 0.95 0.82 0.95] size=5, child 1,2,5
